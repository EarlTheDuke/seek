...
## Added 2026-08-10 08:35, from the pass that ranked this file by its own dagger marker

---

**A301 †††† — THIS LIST HAS CARRIED A PRIORITY MARKER SINCE LINE 12 AND NOTHING HAS EVER SORTED BY IT **[S]**

**The observation.** `IDEAS.md` is now **326 items** across 5,840 lines, and 0 have been built. The
legend at line 12 defines **`†` — "I think this is high value relative to its cost"**, and passes have
been applying 1–5 daggers for weeks alongside `[S]/[M]/[L]`. No pass, and no entry in
`OBSERVATIONS-2026-08-08.md`, has ever read those markers back out. Parsed:

```
  ††††† 9    †††† 21    ††† 59    †† 86    † 40    unrated 111
  [S] 215    [M] 82     [—] 9     unmarked 20
```

The `†††††` ∩ `[S]` intersection is **7 items**: A121, A287, A288, A290, A292, A296, A297.

**Why it matters.** A296 established the backlog is write-only and A297 found its cause (the builder
cron is off). This is the *third* leg: even with the builder switched back on, whoever or whatever
runs it faces an unordered 326-item wall with no entry point, and the natural failure mode is to build
the most recent items rather than the best ones — the last 20 items added are simply the ones nearest
the bottom of the file. The data to prevent that has been in the file the whole time.

**Also: a third of the list is not triageable.** **111 items carry no dagger** and **20 carry no size
marker**. Those are not "low priority" — they are unrated, and they are invisible to any ranking. Most
are early (A0–A60 era), from before the marker convention settled.

**Fix [S].** Two parts, neither touching the game:
1. A `server/rank.mjs` (~30 lines) that parses the headings and prints the list sorted by dagger desc,
   size asc — and separately prints the unrated ones as a "needs a rating" queue. The parse is one
   regex; note that `†` is **3 bytes in UTF-8**, so a byte-oriented `grep -E '†{5}'` silently matches
   nothing (this cost me a false finding in the pass that wrote this item).
2. A `## Start here` block at the **top** of `IDEAS.md` holding the current 7, regenerated by that
   script. The top of the file is the only part a triage pass is guaranteed to read.

**Caveat, stated plainly.** The daggers are self-assigned by the same loop that wrote the items, and
